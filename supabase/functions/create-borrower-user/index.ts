...
Deno.serve(async (req) => {
  ...
  if (action === "reset-password") {
    if (!borrower.auth_user_id) return json({ error: "Borrower does not have a linked portal user." }, 400);
    ...
    if (updateUserError || !updated?.user) {
      return json({ error: updateUserError?.message || "Could not reset temporary password." }, 400);
    }
    return json({
